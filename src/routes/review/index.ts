...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const canReview = await ReviewService.canReviewJob(
        userId,
        reviewData.jobPostId,
      );
      if (!canReview) {
        res.status(403).json({
          success: false,
...
router.get(
  "/healthcare/:healthcareProviderId",
  requireNonAdmin,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const canSeePrivate =
        currentUserId === healthcareProviderId || req.user?.role === "admin";
...
router.get(
  "/healthcare/:healthcareProviderId/stats",
  requireNonAdmin,
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { healthcareProviderId } = req.params;
