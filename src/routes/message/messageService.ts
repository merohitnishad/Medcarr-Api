...
export class MessageService {
  ...
  static async createConversation(data: CreateConversationData) {
    return await db.transaction(async (tx) => {
      ...
      const isHealthcareWorker =
        jobApplication.healthcareUserId === data.healthcareUserId;
  ...
  static async sendMessage(data: SendMessageData) {
    return await db.transaction(async (tx) => {
      ...
      if (
        data.senderId !== conversation.jobPosterId &&
        data.senderId !== conversation.healthcareUserId
      ) {
  ...
  static async getConversationMessages(
    ...
    const otherParticipant =
      conversation.jobPosterId === userId
  ...
  static async getUserConversations(
    ...
    const conversationsWithUnreadCount = await Promise.all(
      results.map(async (conversation) => {
        const lastReadAt =
          userId === conversation.jobPosterId
        ...
        const otherParticipant = isHealthcareUser
          ? {
              ...
              healthcareProfileImage:
                conversation.healthcareUser.healthcareProfile?.image || null,
            }
          : {
  ...
  static async markMessagesAsDelivered(
    ...
    try {
      ...
      const conversationIds: string[] = [
        ...new Set(undeliveredMessages.map((msg) => msg.conversationId)),
      ];
  ...
  static async getOrCreateConversation(
    ...
    if (existingConversation) {
      return {
        ...
        otherParticipant:
          userId === existingConversation.jobPosterId
