...
router.patch(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const updatedConversation = await MessageService.toggleBlockConversation(
        conversationId,
        userId,
        block,
      );
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const updatedConversation =
        await MessageService.toggleArchiveConversation(
          conversationId,
          userId,
          archive,
        );
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const unreadConversations = await MessageService.getUserConversations(
        userId,
        { limit: 100 },
      );
      ...
      res.json({
        ...
        data: {
          ...
          activeConversations:
            ...
            archivedConversations.pagination.total,
        },
      });
      return;
    } catch (error) {
