  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const missingFields = requiredFields.filter(
        (field) => !jobPostData[field as keyof CreateJobPostData],
      );
      ...
      if (jobPostData.careNeedIds && jobPostData.careNeedIds.length > 0) {
        const validCareNeeds = await JobPostService.validateCareNeedIds(
          jobPostData.careNeedIds,
        );
        if (!validCareNeeds) {
          res.status(400).json({
            success: false,
      ...
      if (jobPostData.languageIds && jobPostData.languageIds.length > 0) {
        const validLanguages = await JobPostService.validateLanguageIds(
          jobPostData.languageIds,
        );
        if (!validLanguages) {
          res.status(400).json({
            success: false,
      ...
      if (jobPostData.preferenceIds && jobPostData.preferenceIds.length > 0) {
        const validPreferences = await JobPostService.validatePreferenceIds(
          jobPostData.preferenceIds,
        );
        if (!validPreferences) {
          res.status(400).json({
            success: false,
      ...
      res.status(500).json({
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to create job post",
      });
      return;
    }
  },
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const filters: JobPostFilters = {
        ...
        caregiverGender: toArray(req.query.caregiverGender) as (
          | "male"
          | "female"
        )[],
        shiftType: toArray(req.query.shiftType) as ("day" | "night")[],
        minPaymentCost: req.query.minPaymentCost
          ? parseInt(req.query.minPaymentCost as string)
          : undefined,
        maxPaymentCost: req.query.maxPaymentCost
          ? parseInt(req.query.maxPaymentCost as string)
          : undefined,
        startDate: req.query.startDate as string,
        shiftLengthRanges: req.query.shiftLength
          ? parseShiftLengthRanges(req.query.shiftLength as string)
          : undefined,
      ...
      const sanitizedData = result.data.map((jobPost: any) =>
        JobPostService.sanitizeJobPostData(jobPost),
      );
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const filters: JobPostFilters = {
        ...
        limit: req.query.limit ? parseInt(req.query.limit as string) : 10,
        postcode: req.query.postcode as string,
        type: parseArrayParam(req.query.type) as
          | ("oneDay" | "weekly")[]
          | undefined,
        paymentType: parseArrayParam(req.query.paymentType) as
          | ("hourly" | "fixed")[]
          | undefined,
        caregiverGender: parseArrayParam(req.query.caregiverGender) as
          | ("male" | "female")[]
          | undefined,
        shiftType: parseArrayParam(req.query.shiftType) as ("day" | "night")[],
        minPaymentCost: req.query.minPaymentCost
          ? parseFloat(req.query.minPaymentCost as string)
          : undefined,
        maxPaymentCost: req.query.maxPaymentCost
          ? parseFloat(req.query.maxPaymentCost as string)
          : undefined,
        startDate: req.query.startDate as string,
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const result = await JobPostService.getUserClosedJobPosts(
        userId,
        filters,
      );
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const result = await JobPostService.getUserExpiredJobPosts(
        userId,
        filters,
      );
...
router.patch(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const updatedJobPost = await JobPostService.closeJobPost(
        jobPostId,
        userId,
      );
      ...
      if (
        ...
        res.status(500).json({
          success: false,
          error:
            error instanceof Error ? error.message : "Failed to close job post",
        });
      }
      return;
    }
...
router.delete(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const deletedJobPost = await JobPostService.updateJobPost(
        jobPostId,
        userId,
        {
          isDeleted: true,
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      try {
        ...
        const validationResult = await JobPostService.parseBulkJobData(
          jsonData,
          userId,
        );
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      res.status(500).json({
        success: false,
        error:
          error instanceof Error ? error.message : "Failed to create bulk jobs",
      });
      return;
    }
  },
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const sampleData = [
        {
          ...
          shiftLength: 8,
          overview:
            "Looking for a caring and experienced caregiver to help with daily activities for my elderly mother. She needs assistance with personal care, meal preparation, and companionship.",
        ...
        },
        {
          ...
          shiftLength: 4,
          overview:
            "Seeking a patient and understanding caregiver for my father who has early-stage dementia. Need help with medication reminders, light housekeeping, and social interaction.",
      ...
      if (format === "xlsx") {
        ...
        const referenceData = [
          {
            ...
            value: "",
            description: "Choose from the following gender options:",
          },
          { category: "", value: "male", description: "Male" },
          { category: "", value: "female", description: "Female" },

          {
            category: "PAYMENT TYPE OPTIONS",
            value: "",
            description: "Choose from the following payment types:",
          },
          { category: "", value: "hourly", description: "Pay per hour worked" },
          {
            category: "",
            value: "fixed",
            description: "Fixed amount for the entire job",
          },

          {
            category: "AVAILABLE CARE NEEDS",
            value: "",
            description: "Use these exact names, comma-separated:",
          },
          ...careNeeds.map((cn) => ({
            category: "",
            value: cn.name,
            description: `ID: ${cn.id}`,
          })),
          { category: "", value: "", description: "" },

          {
            category: "AVAILABLE LANGUAGES",
            value: "",
            description: "Use these exact names, comma-separated:",
          },
          ...languages.map((l) => ({
        ...
        res.setHeader(
          "Content-Disposition",
          "attachment; filename=bulk_job_template.xlsx",
        );
        res.send(buffer);
        return;
      } else {
        ...
        res.setHeader(
          "Content-Disposition",
          "attachment; filename=bulk_job_template.csv",
        );
...
function parseShiftLengthRanges(
  shiftLengthParam: string,
