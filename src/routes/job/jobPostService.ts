...
export class JobPostService {
  ...
  static async getUserJobPosts(userId: string, filters: JobPostFilters = {}) {
    ...
    if (jobIds.length > 0) {
      ...
      applicationsByJob = applications.reduce(
        (acc, app) => {
        ...
        if (startHour >= 19) {
          ...
          if (endHour >= 19) {
            ...
            if (
              ...
              errors.push(
                "For night shifts starting in evening, end time must be later than start time",
              );
            }
          }
          ...
          if (endHour < 11) {
            ...
            if (
              ...
              errors.push(
                "For night shifts in morning, end time must be after start time",
              );
            }
          } else if (endHour >= 19) {
            // Starting morning, ending evening - this doesn't make sense for a night shift
            errors.push(
              "Invalid night shift: cannot start in morning and end in evening",
            );
          }
        }
      }
    ...
    if (userRole === "organization") {
      ...
      if (duplicatesWithinBulk.length > 0) {
        ...
        validJobData = validJobData.filter(
          (job) =>
            !duplicatesWithinBulk.some(
              (dup) => dup.rowNumber === job.rowNumber,
            ),
        );
      }
    }
