...
import {
  HealthcareService,
  CreateHealthcareProfileData,
  CreateBankDetailsData,
  PublicHealthcareProfile,
} from "./healthcareService.js";
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const userWithProfile =
        await HealthcareService.getCompleteProfile(userId);
      ...
      const sanitizedData =
        HealthcareService.sanitizeCompleteUserData(userWithProfile);
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const missingFields = requiredFields.filter(
        (field) => !profileData[field as keyof CreateHealthcareProfileData],
      );
      ...
      const validationErrors =
        HealthcareService.validateProfileData(profileData);
      ...
      if (profileData.specialityIds && profileData.specialityIds.length > 0) {
        const validSpecialities = await HealthcareService.validateSpecialityIds(
      ...
      if (profileData.languageIds && profileData.languageIds.length > 0) {
        const validLanguages = await HealthcareService.validateLanguageIds(
      ...
      const createdProfile = await HealthcareService.createProfile(
        userId,
        profileData,
      );
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      // Validate profile data
      const validationErrors =
        HealthcareService.validateProfileData(updateData);
      if (validationErrors.length > 0) {
        res.status(400).json({
          success: false,
          error: "Validation failed",
      ...
      const updatedProfile = await HealthcareService.updateProfile(
        userId,
        updateData,
      );
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const updatedUser = await HealthcareService.updateBasicInfo(
        userId,
        updateData,
      );
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const hasAccess = await HealthcareService.validateUserAccess(
        currentUserId,
        userId,
      );
...
router.post(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const missingFields = requiredFields.filter(
        (field) => !bankData[field as keyof CreateBankDetailsData],
      );
      ...
      const createdBankDetails = await HealthcareService.createBankDetails(
        userId,
        bankData,
      );
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      // Validate bank details
      const validationErrors =
        HealthcareService.validateBankDetails(updateData);
      if (validationErrors.length > 0) {
        res.status(400).json({
          success: false,
          error: "Validation failed",
      ...
      const updatedBankDetails = await HealthcareService.updateBankDetails(
        userId,
        updateData,
      );
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const userWithProfile =
        await HealthcareService.getCompleteProfileWithBankDetails(userId);
...
router.get(
  "/public/:healthcareProviderId",
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      const { healthcareProviderId } = req.params;

      const userWithProfile =
        await HealthcareService.getCompleteProfileWithReviews(
      ...
      if (sanitizedData.healthcareProfile) {
        const { phoneNumber, address, bankDetails, ...publicProfileData } =
