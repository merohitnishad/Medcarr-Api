...
export class OrganizationService {
  ...
  static async getBasicProfile(userId: string): Promise<User | null> {
    try {
      const result = await db
        ...
        .from(users)
        .where(and(eq(users.id, userId), eq(users.role, "organization")))
        .limit(1);
  ...
  static async getCompleteProfile(
    ...
    try {
      ...
      if (result.organizationProfile) {
        const { overview: overviewRaw, ...restProfile } =
  ...
  static async createProfile(
    userId: string,
    profileData: CreateOrganizationProfileData,
  ...
  static async updateProfile(
    userId: string,
    profileData: Partial<CreateOrganizationProfileData>,
  ): Promise<OrganizationProfile> {
    try {
  ...
  static validateProfileData(
    data: Partial<CreateOrganizationProfileData>,
  ): string[] {
    const errors: string[] = [];
    ...
    if (
      ...
      (!data.organizationType || data.organizationType.trim().length < 2)
    ) {
