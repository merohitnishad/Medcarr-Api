  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const userWithProfile =
        await OrganizationService.getCompleteProfile(userId);
      ...
      const sanitizedData =
        OrganizationService.sanitizeCompleteUserData(userWithProfile);
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const missingFields = requiredFields.filter(
        (field) => !profileData[field as keyof CreateOrganizationProfileData],
      );
      ...
      const validationErrors =
        OrganizationService.validateProfileData(profileData);
      ...
      const createdProfile = await OrganizationService.createProfile(
        userId,
        profileData,
      );
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      // Validate profile data
      const validationErrors =
        OrganizationService.validateProfileData(updateData);
      if (validationErrors.length > 0) {
        res.status(400).json({
          success: false,
          error: "Validation failed",
      ...
      const updatedProfile = await OrganizationService.updateProfile(
        userId,
        updateData,
      );
...
router.put(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const updatedUser = await OrganizationService.updateBasicInfo(
        userId,
        updateData,
      );
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const hasAccess = await OrganizationService.validateUserAccess(
        currentUserId,
        userId,
      );
