...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const result = await JobApplicationService.getHealthcareApplications(
        healthcareUserId,
        filters,
      );
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const filters: ApplicationFilters = {
        ...
        reviewLimit: req.query.reviewLimit
          ? parseInt(req.query.reviewLimit as string)
          : undefined,
        reviewOffset: req.query.reviewOffset
          ? parseInt(req.query.reviewOffset as string)
          : undefined,
      };
...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      const result = await JobApplicationService.getAllUserJobApplications(
        userId,
        filters,
      );
...
router.patch(
  ...
  async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (
        !statusData.status ||
        !["accepted", "rejected"].includes(statusData.status)
      ) {
        res.status(400).json({
          success: false,
