...
import { createUserSchema, users } from "../../db/schemas/usersSchema.js";
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      if (
        roleFromCognito === "admin" ||
        roleFromCognito === "individual" ||
        roleFromCognito === "organization" ||
        roleFromCognito === "healthcare"
      ) {
        role = roleFromCognito as
          | "admin"
          | "individual"
          | "organization"
          | "healthcare";
      ...
      const existingUser = await db
        .select()
        .from(users)
        .where(and(eq(users.cognitoId, cognitoId), eq(users.isDeleted, false)))
        .limit(1);
