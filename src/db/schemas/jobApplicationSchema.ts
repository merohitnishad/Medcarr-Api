...
export const jobApplications = pgTable(
  ...
  (table) => ({
    ...
    healthcareUserIdIdx: index("job_applications_healthcare_user_id_idx").on(
...
export const jobApplicationPreferences = pgTable(
  ...
  (table) => ({
    jobApplicationIdIdx: index(
      "job_application_preferences_application_id_idx",
    ).on(table.jobApplicationId),
    preferenceIdIdx: index("job_application_preferences_preference_id_idx").on(
      table.preferenceId,
    ),
    // Unique constraint to prevent duplicate entries
    uniqueJobApplicationPreference: index(
      "unique_job_application_preference",
    ).on(table.jobApplicationId, table.preferenceId),
  }),
);
...
export const jobApplicationPreferencesRelations = relations(
...
export const createJobApplicationSchema = createInsertSchema(
  jobApplications,
).omit({
  id: true,
  status: true,
...
export const updateApplicationStatusSchema = createInsertSchema(
  jobApplications,
).pick({
  status: true,
  responseMessage: true,
