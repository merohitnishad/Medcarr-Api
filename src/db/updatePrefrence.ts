...
const updatePreferences = async () => {
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      for (
        let i = 0;
        i < existingPreferences.length && i < updatedPreferencesData.length;
        i++
      ) {
    ...
    console.log(
      `✨ Successfully updated ${Math.min(existingPreferences.length, updatedPreferencesData.length)} preferences!`,
    );
  } catch (error) {
