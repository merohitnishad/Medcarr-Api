...
import { IndividualService, CreateIndividualProfileData } from './individualService.js';

const router = Router();

// Get individual user's basic profile (just user table data)
router.get('/profile/basic', individualOnly, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.get('/profile', individualOnly, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const userWithProfile = await IndividualService.getCompleteProfile(userId);
...
router.post('/profile/complete', individualOnly, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const validationErrors = IndividualService.validateProfileData(profileData);
    ...
    const existingProfile = await IndividualService.getCompleteProfile(userId);
...
router.put('/profile', individualOnly, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Validate profile data
    const validationErrors = IndividualService.validateProfileData(updateData);
    if (validationErrors.length > 0) {
      res.status(400).json({ 
        success: false,
        error: 'Validation failed',
    ...
    const updatedProfile = await IndividualService.updateProfile(userId, updateData);
...
router.put('/profile/basic', individualOnly, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    ...
    const updatedUser = await IndividualService.updateBasicInfo(userId, updateData);
...
router.get('/profile/status', individualOnly, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
