...
router.get('/dashboard/stats', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.get('/dashboard/user-distribution', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const adminId = req.user!.id;
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to fetch user distribution'
    });
    return;
  }
});
...
// Get recent activity
router.get('/dashboard/recent-activity', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const adminId = req.user!.id;
    const { limit = 10 } = req.query;
    
    const activity = await AdminService.getRecentActivity(adminId, parseInt(limit as string));
...
router.get('/user-requests', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const {
      page = 1,
      limit = 20,
...
// Verify user DBS
router.post('/user-requests/:userId/verify-dbs', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (error instanceof Error && error.message.includes('Access denied')) {
      ...
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to verify DBS'
      });
    }
    return;
  }
...
// Verify user profile
router.post('/user-requests/:userId/verify-profile', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { userId } = req.params;
    const adminId = req.user!.id;
...
router.get('/users/individuals', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const {
      page = 1,
      limit = 20,
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to fetch individuals'
    });
    return;
  }
});
...
router.get('/users/organizations', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const {
      page = 1,
      limit = 20,
...
router.get('/users/healthcare', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const {
      page = 1,
      limit = 20,
    ...
    res.status(500).json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to fetch healthcare providers'
    });
    return;
  }
});
...
// Get all disputes
router.get('/disputes', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const {
      page = 1,
      limit = 20,
...
router.patch('/disputes/:disputeId/status', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!statusData.status || !['in_review', 'resolved', 'dismissed'].includes(statusData.status)) {
      res.status(400).json({
        success: false,
    ...
    if ((statusData.status === 'resolved' || statusData.status === 'dismissed') && !statusData.resolutionDescription) {
      res.status(400).json({
        success: false,
        error: 'Resolution description is required for resolved or dismissed disputes'
      });
      return;
    }
    ...
    if (error instanceof Error && (error.message === 'Dispute not found' || error.message.includes('Access denied'))) {
      res.status(404).json({
        success: false,
        error: error.message
      });
...
router.get('/disputes/:disputeId/conversation', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { disputeId } = req.params;
    const adminId = req.user!.id;

    const conversation = await AdminService.getDisputeConversation(adminId, disputeId);
    ...
    if (error instanceof Error && error.message.includes('Access denied')) {
    ...
    } else if (error instanceof Error && error.message === 'Dispute not found') {
      res.status(404).json({
        success: false,
        error: error.message
      });
      ...
      res.status(500).json({
        success: false,
        error: error instanceof Error ? error.message : 'Failed to fetch dispute conversation'
      });
    }
    return;
  }
...
router.get('/notifications', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const adminId = req.user!.id;
    const {
      page = 1,
...
router.patch('/notifications/:notificationId/read', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { notificationId } = req.params;
    const adminId = req.user!.id;

    const success = await AdminService.markNotificationAsRead(adminId, notificationId);
...
router.patch('/notifications/read-all', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const adminId = req.user!.id;

    const updatedCount = await AdminService.markAllNotificationsAsRead(adminId);
...
router.get('/profile', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const adminId = req.user!.id;
