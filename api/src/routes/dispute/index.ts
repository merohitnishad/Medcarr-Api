...
import { requireAdminRole, requireNonAdmin } from '../../middlewares/roleAuth.js';
import { 
  DisputeService,
  CreateDisputeData,
...
router.post('/create', requireNonAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!disputeData.jobPostId || !disputeData.reportedAgainst || !disputeData.disputeType || !disputeData.title || !disputeData.description) {
      res.status(400).json({
        success: false,
        error: 'All fields are required: jobPostId, reportedAgainst, disputeType, title, description'
      });
      return;
    }
...
router.get('/my-disputes', requireNonAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user!.id;
...
router.get('/:disputeId', requireNonAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { disputeId } = req.params;
    const userId = req.user!.id;
    const userRole = req.user!.role;

    const dispute = await DisputeService.getDispute(disputeId, userId, userRole);
    ...
    if (error instanceof Error && (error.message === 'Dispute not found' || error.message === 'Access denied')) {
      res.status(404).json({
        success: false,
        error: error.message
      });
...
router.post('/:disputeId/upload-url', requireNonAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { disputeId } = req.params;
    const userId = req.user!.id;
...
router.post('/:disputeId/confirm-upload', requireNonAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { disputeId } = req.params;
    const userId = req.user!.id;
    ...
    // Validate required fields
    if (!documentData.fileName || !documentData.originalFileName || !documentData.contentType) {
      res.status(400).json({
        success: false,
...
// Delete document
router.delete('/documents/:documentId', requireNonAdmin, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { documentId } = req.params;
    const userId = req.user!.id;
    const userRole = req.user!.role;

    const result = await DisputeService.deleteDocument(documentId, userId, userRole);
    ...
    if (error instanceof Error && (error.message === 'Document not found' || error.message === 'Access denied')) {
      res.status(404).json({
        success: false,
        error: error.message
      });
...
router.get('/admin/all', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const filters: DisputeFilters = {
      page: req.query.page ? parseInt(req.query.page as string) : 1,
      limit: req.query.limit ? parseInt(req.query.limit as string) : 10,
...
router.patch('/:disputeId/status', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { disputeId } = req.params;
...
router.get('/admin/stats', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.get('/admin/:disputeId', requireAdminRole, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { disputeId } = req.params;
    const adminId = req.user!.id;

    const dispute = await DisputeService.getDispute(disputeId, adminId, 'admin');
